...
class UPerNetDecoder(Decoder):
    ...
    def forward(self, *features):
        ...
        #  *feature:两幅输入送入相同encoders后得到的两组不同尺度特征图[features_A, features_B]
        ...
        c5 = self.psp(c5)
        print("c5", c5.shape)
        #  通道数统一调整为pyramid_channels
        p5 = self.psp_last_conv(c5)  # torch.Size([8, 256, 8, 8])
        print("p5", p5.shape)
        p4 = self.p4(p5, c4)  # torch.Size([8, 256, 16, 16])
        print("p4", p4.shape)
        p3 = self.p3(p4, c3)  # torch.Size([8, 256, 32, 32])
        print("p3", p3.shape)
        p2 = self.p2(p3, c2)  # torch.Size([8, 256, 64, 64])
        print("p2", p2.shape)

        output_size = p2.size()[2:]

        #  对最终的多尺度的特征图进行原尺度的上采样后进行维度的堆叠
        #  上采样
